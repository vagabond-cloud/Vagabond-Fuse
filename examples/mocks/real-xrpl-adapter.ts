...
export class RealXRPLAdapter {
  ...
  async connect(): Promise<boolean> {
    ...
    try {
      ...
      this.spinner.fail(chalk.red(`Failed to connect to XRPL ${this.network}: ${error.message}`));
      throw error;
    }
  }
  ...
  async disconnect(): Promise<boolean> {
    this.spinner.start(chalk.blue(`Disconnecting from XRPL ${this.network}...`));
    try {
      await this.client.disconnect();
      this.connected = false;
      this.spinner.succeed(chalk.green(`Disconnected from XRPL ${this.network}`));
      return true;
    } catch (error: any) {
      this.spinner.fail(chalk.red(`Failed to disconnect from XRPL ${this.network}: ${error.message}`));
      throw error;
    }
  }
  ...
  async createWallet(userId: string): Promise<any> {
    ...
    try {
      ...
      this.spinner.succeed(chalk.green(`Created and funded XRPL wallet for ${userId}`));
      ...
      this.spinner.fail(chalk.red(`Failed to create XRPL wallet: ${error.message}`));
      throw error;
    }
  }
  ...
  async sendPayment(
    ...
    this.spinner.start(chalk.blue(`Sending ${amountInXRP} XRP to ${recipientAddress}...`));
    ...
    try {
      ...
      const txResult = txResponse.result.meta && typeof txResponse.result.meta === 'object' ? 
        txResponse.result.meta.TransactionResult : 'Unknown';
      
      if (txResult === 'tesSUCCESS') {
        this.spinner.succeed(chalk.green(`Payment of ${amountInXRP} XRP sent successfully`));
        console.log(chalk.cyan(`  Transaction hash: ${txResponse.result.hash}`));
        console.log(chalk.cyan(`  Explorer link: https://${this.network === 'mainnet' ? '' : this.network + '.'}xrpl.org/transactions/${txResponse.result.hash}`));
        ...
        return {
          ...
          amount: amount,
          fee: Number(txResponse.result.Fee),
  ...
  async issueToken(
    ...
    try {
      ...
      const existingRecipient = Array.from(this.wallets.values())
        .find(wallet => wallet.address === recipientAddress);
      ...
      this.spinner.succeed(chalk.green(`Trustline established for ${tokenCode}`));
      
      // Now issue the token by sending a payment
      this.spinner.start(chalk.blue(`Issuing ${amount} ${tokenCode} tokens to ${recipientAddress}...`));
      ...
      const txResult = paymentResult.result.meta && typeof paymentResult.result.meta === 'object' ? 
        paymentResult.result.meta.TransactionResult : 'Unknown';
      
      if (txResult === 'tesSUCCESS') {
        this.spinner.succeed(chalk.green(`Issued ${amount} ${tokenCode} tokens successfully`));
        console.log(chalk.cyan(`  Transaction hash: ${paymentResult.result.hash}`));
        console.log(chalk.cyan(`  Explorer link: https://${this.network === 'mainnet' ? '' : this.network + '.'}xrpl.org/transactions/${paymentResult.result.hash}`));
