/**
 * Example demonstrating how to use the Policy Engine service with OPA WASM integration in JavaScript
 *
...
function mockCreatePolicy(policy) {
...
async function evaluatePolicy(
    ...
    // Force mock mode for this example to ensure consistent behavior
    MOCK_MODE = true;
...
function mockEvaluatePolicy(data) {
    ...
    // Determine if allowed based on GDPR rules
    // All conditions must be true for access to be allowed
    const allowed =
        hasConsent &&
        hasPurpose &&
        isPurposeAllowed &&
        isSensitivityAcceptable;

    // Generate reasons
    const reasons = ["Local evaluation: No policy engine available"];
...
function mockUpdatePolicy(policy) {
    console.log('MOCK: Updated policy');

    // Create a structure that matches the API response
    return {
        policy_id: policy.id || `policy-${uuidv4()}`,
        policy: {
            ...policy,
            version: policy.version || '1.0.0', // Ensure version is defined
            rules: policy.rules || [], // Ensure rules is defined and is an array
        },
...
function mockDeletePolicy(policyId) {
...
// Main function
async function main() {
    console.log('Policy Engine Service WASM Example (JavaScript)');
    console.log('--------------------------------------------');
    ...
    if (updateResult) {
        ...
        // Handle both direct and nested policy structures
        const policy = updateResult.policy || updateResult;

        console.log(`  New version: ${policy.version}`);

        // Check if rules exists before accessing length
        if (policy.rules) {
            console.log(`  Rules count: ${policy.rules.length}`);
        } else {
            console.log('  Rules: not available');
        }
    }
    ...
    console.log(`\n5. Cleaning up - deleting policy ${policyId}...`);

    const deleteResult = await deletePolicy(policyId);
